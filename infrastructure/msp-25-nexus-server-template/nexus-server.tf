...
resource "null_resource" "forpasswd" {
  ...
  provisioner "local-exec" {
    command = "ssh -i /Users/abdullahyalcin/Downloads/${var.mykey}.cer -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ec2-user@${aws_instance.tf-nexus-server.public_ip} 'docker cp nexus:/nexus-data/admin.password  admin.password && cat /home/ec2-user/admin.password' > initialpasswd.txt"
  }
}
